...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900">Modifier le témoignage</h1>
</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="rating" required
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#3B7BF8] focus:border-transparent">
                    @for($i = 5; $i >= 1; $i--)
                    <option value="{{ $i }}" {{ $testimonial->rating == $i ? 'selected' : '' }}>{{ $i }} étoile{{ $i > 1 ? 's' : '' }}</option>
                    @endfor
                </select>
            </div>
            ...
            <div>
                ...
                <input type="number" name="display_order" value="{{ old('display_order', $testimonial->display_order) }}" required
                       class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#3B7BF8] focus:border-transparent">
            </div>
            
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Couleur de fond *</label>
                <input type="color" name="background_color" value="{{ old('background_color', $testimonial->background_color) }}" required
            ...
            <div class="md:col-span-2">
                ...
                <div class="mb-2">
                    <img src="{{ asset('storage/' . $testimonial->photo) }}" alt="" class="w-20 h-20 rounded-full object-cover">
                </div>
                @endif
            ...
            <div class="md:col-span-2">
                ...
                @if($testimonial->icon)
                <div class="mb-2">
                    <img src="{{ asset('storage/' . $testimonial->icon) }}" alt="" class="w-16 h-16 object-cover">
                </div>
                @endif
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="testimonial" rows="5" required
                          class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#3B7BF8] focus:border-transparent">{{ old('testimonial', $testimonial->testimonial) }}</textarea>
            </div>
